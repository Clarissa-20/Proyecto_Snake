...
void Nivel::volverAlMenu()
{
    ...
    else if (usuarioRetorno.isEmpty()==false)
    {
        MenuPrincipal *menu= new MenuPrincipal(nullptr, usuarioRetorno);
        menu->show();
    }
    this->close();
}
...
void Nivel::cargarSkinUsuario() {
    if (usuarioActual.isEmpty()) return;
    std::string userStr = usuarioActual.toStdString();
    Usuario u;
    if (UserManager::cargarDatosUsuario(userStr, u)) {
        skinActualNivel = u.skinActual;
    }
}
...
{
    if (event->key() == Qt::Key_Escape && gameover == false)
    {
        if (pausaBtn->isVisible() == true)
        {
            pausaBtn->click();
        }
        else if (btnReaunudar->isVisible() == true)
        {
            btnReaunudar->click();
        }
        return;
...
void Nivel::setUsuarioRetorno(const QString &usuario)
{
